...
import { updateSite } from "../__requests/apiRequests";
...
const EditSiteModal: React.FC<EditSiteModalProps> = ({
  ...
  useEffect(() => {
    ...
    const loadCurrentVille = async () => {
      try {
        const response = await fetchVilles(1, "");
        if (response && response.items) {
          const ville = response.items.find((v: Ville) => v.ville_Id === currentVilleId);
          if (ville) {
            setCurrentVille(ville);
            setSelectedPaysId(ville.pays_Id.toString());
          }
        }
      } catch (error) {
        console.error("Erreur lors du chargement de la ville actuelle:", error);
      }
    };
  ...
  const formik = useFormik({
    initialValues: {
      site_Nom: currentNom,
      site_Adresse: currentAdresse,
      site_PaysId: currentVille?.pays_Id.toString() || "",
      site_VilleId: currentVilleId.toString(),
      site_Actif: currentActif,
    },
    ...
    onSubmit: async (values) => {
      ...
      try {
        await updateSite(
          siteId,
          values.site_Nom.trim(),
          values.site_Adresse.trim(),
          parseInt(values.site_VilleId, 10),
          values.site_Actif
        );
        Swal.fire("Succès", "Le site a été modifié avec succès", "success");
        onUpdated();
        onClose();
      } catch (err) {
        ...
        <div className="modal-content shadow">
          ...
          <form onSubmit={formik.handleSubmit}>
            <div className="modal-body">
              <div className="mb-3">
                ...
                <input
                  ...
                  className={`form-control ${
                    formik.errors.site_Nom && formik.touched.site_Nom ? "is-invalid" : ""
                  }`}
                  onChange={formik.handleChange}
                  value={formik.values.site_Nom}
                />
                {formik.errors.site_Nom && formik.touched.site_Nom && (
                  <div className="invalid-feedback">{formik.errors.site_Nom}</div>
                )}
              </div>
              ...
              <div className="mb-3">
                ...
                <input
                  ...
                  name="site_Adresse"
                  className={`form-control ${
                    formik.errors.site_Adresse && formik.touched.site_Adresse ? "is-invalid" : ""
                  }`}
                  onChange={formik.handleChange}
                  value={formik.values.site_Adresse}
                />
                {formik.errors.site_Adresse && formik.touched.site_Adresse && (
                  <div className="invalid-feedback">{formik.errors.site_Adresse}</div>
                )}
              </div>
              ...
              <div className="mb-3">
                ...
                {formik.errors.site_PaysId && formik.touched.site_PaysId && (
                  <div className="invalid-feedback">{formik.errors.site_PaysId}</div>
                )}
                {loadingPays && (
              ...
              <div className="mb-3">
                ...
                {formik.errors.site_VilleId && formik.touched.site_VilleId && (
                  <div className="invalid-feedback">{formik.errors.site_VilleId}</div>
                )}
                {loadingVilles && (
                  <div className="text-muted mt-1">
            ...
            <div className="modal-footer">
              ...
              <button type="submit" className="btn btn-primary" disabled={loading || loadingPays || loadingVilles}>
